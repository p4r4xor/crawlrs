...
// ---------------------------------------------------------------------------
// has_any_quota: the predicate the politeness rate limiter
// consults to skip per-host counter writes when no quota anywhere
// ---------------------------------------------------------------------------

#[test]
fn has_any_quota_false_for_empty_scope() {
    let s = CrawlScope::default();
    assert!(!s.has_any_quota());
}

#[test]
fn has_any_quota_true_when_global_set() {
    let s = scope(None, Some(1000));
    assert!(s.has_any_quota());
}

#[test]
fn has_any_quota_true_when_only_per_domain_set() {
    let s = scope_with_override(
        None,
        None,
        "capped.test",
...
#[test]
fn has_any_quota_false_when_per_domain_only_sets_max_depth() {
    // Only `max_urls` matters for quota; a `max_depth`-only override
    // doesn't count.
    let s = scope_with_override(
        None,
        None,
